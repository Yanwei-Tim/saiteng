...
int  MD5Sum::getString(char * _Drc,int start,int end)
{
	if(_Drc == NULL) return 1;
	commit();
	char buf[36];
	int i;

	for(i = 0; i < 16; ++i)
		sprintf_s(buf + 2 * i,3, "%02x", m_md5[i]);
	buf[32] = 0;
	end = end < 32?end:32;
...
int  MD5Sum::getTempString(char * _Drc,int start,int end)
{
	char buf[512];
	//save state
	memcpy_s(buf, 512, this, sizeof(MD5Sum));
	int result = getString(_Drc, start, end);
	//restore state
	memcpy_s(this, sizeof(MD5Sum), buf, 512);
	return result;
}
